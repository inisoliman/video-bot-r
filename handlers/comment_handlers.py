...
def show_user_comments(bot, message, page=0):
    ...
    try:
        ...
        if total > db.VIDEOS_PER_PAGE:
            ...
            if page > 0:
                buttons.append(inline_button("◀️ السابق", STYLE_PRIMARY, callback_data=f"my_comments::{page-1}"))
            
            buttons.append(inline_button(f"📄 {page+1}/{(total-1)//db.VIDEOS_PER_PAGE + 1}", STYLE_PRIMARY, callback_data="noop"))
            
            if (page + 1) * db.VIDEOS_PER_PAGE < total:
                buttons.append(inline_button("التالي ▶️", STYLE_PRIMARY, callback_data=f"my_comments::{page+1}"))
    ...
    try:
        ...
        for comment in comments:
            ...
            if not comment['admin_reply']:
                buttons.append(inline_button("✍️ رد 🟢", STYLE_SUCCESS, callback_data=f"reply_comment::{comment['id']}"))
            
            if not comment['is_read']:
                buttons.append(inline_button("✓ مقروء 🔵", STYLE_PRIMARY, callback_data=f"mark_read::{comment['id']}"))
            
            buttons.append(inline_button("🗑️ حذف 🔴", STYLE_DANGER, callback_data=f"delete_comment::{comment['id']}"))
        ...
        if total > db.VIDEOS_PER_PAGE or not unread_only:
            ...
            if page > 0:
                ...
                nav_buttons.append(inline_button("◀️ السابق", STYLE_PRIMARY, callback_data=callback))
            
            nav_buttons.append(inline_button(f"📄 {page+1}/{(total-1)//db.VIDEOS_PER_PAGE + 1}", STYLE_PRIMARY, callback_data="noop"))
            
            if (page + 1) * db.VIDEOS_PER_PAGE < total:
                callback = f"admin_comments_unread::{page+1}" if unread_only else f"admin_comments::{page+1}"
                nav_buttons.append(inline_button("التالي ▶️", STYLE_PRIMARY, callback_data=callback))
            
            if nav_buttons:
                markup.row(*nav_buttons)
            
            # زر التبديل بين الكل وغير المقروءة
            filter_button = inline_button(
...
def handle_delete_comment(bot, call, admin_ids):
    ...
    try:
        ...
        markup = types.InlineKeyboardMarkup()
        markup.row(
            inline_button("🗑️ تأكيد الحذف 🔴", STYLE_DANGER, callback_data=f"confirm_delete_comment::{comment_id}"),
...
def handle_delete_all_comments(bot, user_id, admin_ids):
    ...
    try:
        ...
        markup = types.InlineKeyboardMarkup()
        markup.row(
            inline_button("🗑️ حذف الكل 🔴", STYLE_DANGER, callback_data="confirm_delete_all_comments"),
            inline_button("↩️ إلغاء 🟢", STYLE_SUCCESS, callback_data="noop")
        )
...
def handle_delete_user_comments(bot, user_id, admin_ids, target_user_id_str=None):
    ...
    try:
        ...
        markup = types.InlineKeyboardMarkup()
        markup.row(
            inline_button("🗑️ تأكيد الحذف 🔴", STYLE_DANGER, callback_data=f"confirm_delete_user_comments::{target_user_id}"),
            inline_button("↩️ إلغاء 🟢", STYLE_SUCCESS, callback_data="noop")
        )
...
def handle_delete_old_comments(bot, user_id, admin_ids, days_str=None):
    ...
    try:
        ...
        markup = types.InlineKeyboardMarkup()
        markup.row(
            inline_button("🗑️ تأكيد الحذف 🔴", STYLE_DANGER, callback_data=f"confirm_delete_old_comments::{days}"),
            inline_button("↩️ إلغاء 🟢", STYLE_SUCCESS, callback_data="noop")
        )
