        ...
        try:
            ...
            if action != "check_subscription" and not is_subscribed:
                ...
                for channel in unsub_channels:
                    ...
                    # إذا كان يبدأ بـ @ فهو username (قناة عامة)
                    if channel_id_str.startswith('@'):
                        link = f"https://t.me/{channel_id_str[1:]}"  # إزالة @ من البداية
                    # إذا كان رقماً سالباً يبدأ بـ -100 (قناة خاصة/مجموعة)
                    elif channel_id_str.startswith('-100'):
                        clean_id = channel_id_str.replace('-100', '', 1)
                        link = f"https://t.me/c/{clean_id}/1"
                    # إذا كان username بدون @ (قناة عامة)
                    elif not channel_id_str.startswith('-') and not channel_id_str.isdigit():
                        link = f"https://t.me/{channel_id_str}"
                    # حالة افتراضية
                    else:
                        if channel_id_str.startswith('-'):
                            clean_id = channel_id_str.replace('-', '', 1)
                            link = f"https://t.me/c/{clean_id}/1"
                        else:
                            link = f"https://t.me/c/{channel_id_str}/1"
